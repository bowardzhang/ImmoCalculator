(function () {
  ...
  function run(clamp){if(clamp)clampDownPayment();var inp=getInputs();if(inp.downPayment>=inp.purchasePrice*(1+inp.grunderwerbsteuer+inp.notar+inp.makler)){alert(t('alert_dp'));return;}var result=calculate(inp);renderKPI(result.data,result.monthlyPayment,result.totalAcqCost);renderChart(result.data);renderTable(result.data);}
  ...
  document.getElementById('headerCalcBtn').addEventListener('click',function(e){e.preventDefault();run(true);});
